...
public abstract class Traversal {
    ...
    protected Traversal(Graph G, Queue<Integer> fringe) {
        _G = G;
        _fringe = fringe;
        allMarked = new ArrayList<Integer>();
        ...
        while (!_fringe.isEmpty()) {
            ...
            if (v < 0) {
                ...
                if (reverseSuccessors(v)) {
                    ...
                    ArrayList<Integer> revSuccessors = new ArrayList<Integer>();
                    for (int x : _G.successors(v)) {
                        revSuccessors.add(0, x);
                    }
                    for (int y : revSuccessors) {
                        if (processSuccessor(v, y)) {
                            _fringe.add(y);
                        }
                    }
                } else {
                    for (int y : _G.successors(v)) {
                        if (processSuccessor(v, y)) {
                            _fringe.add(y);
                        }
                    }
    ...
    /** Returns the list of all postvisited vertices.
    @return getter method that returns postvisited. */
    protected ArrayList<Integer> getVis() {
        return postVisited;
    }

    /** Returns the list of all marked vertices.
    @return Getter method to return allMarked. */
    protected ArrayList<Integer> getMarked() {
        return allMarked;
    }
    ...
    /** The list of the marked status of all vertices. */
    private ArrayList<Integer> allMarked;
    /** The list that tracks postvisited vertices. */
