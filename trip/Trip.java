...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        for (int i = 0; i < segment.size() - 1; i += 1) {
            if (i == segment.size() - 2) {
                Road current = _map.getLabel(segment.get(i),
                    segment.get(i + 1));
                totDist += current.length();
                String currDir = current.direction().fullName();
                System.out.printf("%d. Take %s %s for %.1f miles to %s.",
                    seq, current, currDir, totDist, _map.getLabel(dest));
                System.out.println();
                seq += 1;
            } else {
                Road current = _map.getLabel(segment.get(i),
                    segment.get(i + 1));
                Road next = _map.getLabel(segment.get(i + 1),
                    segment.get(i + 2));
                String currName = current.toString();
                String nextName = next.toString();
                String currDir = current.direction().fullName();
                String nextDir = next.direction().fullName();
                if (currName.equals(nextName) && currDir.equals(nextDir)) {
                    totDist += current.length();
                } else {
                    totDist += current.length();
                    System.out.printf("%d. Take %s %s for %.1f miles.",
                        seq, current, currDir, totDist);
                    System.out.println();
                    seq += 1;
                    totDist = 0.0;
                }
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road curr = new Road(name, dir, length);
        Road opp = new Road(name, dir.reverse(), length);
        _map.add(v0, v1, curr);
        _map.add(v1, v0, opp);
    }
